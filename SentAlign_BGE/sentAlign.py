...
def load_labse_model(proc_device, labse_model):
    ...
    tokenizer = AutoTokenizer.from_pretrained(labse_model)
    model = AutoModel.from_pretrained(labse_model, use_safetensors=True)
    model = model.eval()
    if proc_device == 'cuda':
        model.half()  # Convierte a precisión media (FP16)
...
def emb_file(in_list, in_numpy, current_max_length):
    ctr = 0
    for i in in_list:
        ctr += 1
        inputs = tokenizer(i, return_tensors="pt", padding=True, truncation=True, max_length=current_max_length, add_special_tokens = True)
...
def create_emb_file(infile, maximum_length):
    ...
    while temp_max_length > 30: #arbitrary number, allow user to select in arguments?
        try:
            ...
            embs_numpy = emb_file(sentence_list, embs_numpy, int(temp_max_length))
            break
...
def score_labse_matrix(source_list, target_list):
    source_inputs = tokenizer(source_list, return_tensors="pt", padding=True, truncation=True, max_length=max_length, add_special_tokens=True)
    source_inputs = source_inputs.to(args.proc_device)
    target_inputs = tokenizer(target_list, return_tensors="pt", padding=True, truncation=True, max_length=max_length, add_special_tokens=True)
...
if __name__ == '__main__':
    ...
    tokenizer, model = load_labse_model(args.proc_device, 'BAAI/bge-m3')
    files2align = {}
    filesdone = {}
