...
SentAlign is a sentence alignment tool for parallel corpora. This variant uses [SONAR](https://ai.meta.com/research/no-language-left-behind/) embeddings to find sentence pairs that are similar in meaning 
...
If needed, pass explicit SONAR language tags (recommended for non-Latin scripts):

```bash
python3 sentAlign.py -dir /path/to/files -sl eng -tl isl -slsonar eng_Latn -tlsonar isl_Latn
```

The aligner generates two type of files in the output folder: `/path/to/files/output`. Files ending with `.path` files containing the alignments and SONAR scores. They are formatted like this:
...
and `.aligned` files containing the aligned sentence pairs as well as the SONAR score. Source sentence in the first column, target sentence in the second and SONAR score in the third:
...
'--embedding-model', '-embmodel', default='text_sonar_basic_encoder'
'--embedding-tokenizer', '-embtokenizer', default='text_sonar_basic_encoder'
'--embedding-batch-size', '-embbatch', type=int, default=128
'--source-language-sonar', '-slsonar', default='<source-language>_Latn'
'--target-language-sonar', '-tlsonar', default='<target-language>_Latn'
```
