...
parser.add_argument('--embedding-model', '-embmodel', help='SONAR encoder model name', default='text_sonar_basic_encoder')
parser.add_argument('--embedding-tokenizer', '-embtokenizer', help='SONAR tokenizer model name', default='text_sonar_basic_encoder')
parser.add_argument('--embedding-batch-size', '-embbatch', type=int, help='Batch size when creating SONAR embeddings', default=128)
parser.add_argument('--source-language-sonar', '-slsonar', help='SONAR source language code (for example eng_Latn). Defaults to <source-language>_Latn', default=None)
parser.add_argument('--target-language-sonar', '-tlsonar', help='SONAR target language code (for example isl_Latn). Defaults to <target-language>_Latn', default=None)
...
embedding_tokenizer_name = args.embedding_tokenizer
embedding_file_suffix = '.sonar_emb'

LANG_FOLDER_TO_SONAR = {
    'en': 'eng_Latn',
    'es': 'spa_Latn',
    'fr': 'fra_Latn',
    'it': 'ita_Latn',
...
def infer_language_from_corpus_folder(folder_path):
    return os.path.basename(os.path.normpath(folder_path.strip()))

def infer_sonar_language(language_code):
    normalized = language_code.strip()
    if normalized in {'wiki', 'viki'}:
        folder_lang = infer_language_from_corpus_folder(args.corpus_folder)
        return LANG_FOLDER_TO_SONAR.get(folder_lang, 'eng_Latn')
    if '_' in normalized:
        return normalized
    return normalized + '_Latn'

source_sonar_language = args.source_language_sonar or infer_sonar_language(args.source_language)
target_sonar_language = args.target_language_sonar or infer_sonar_language(args.target_language)
...
def load_sonar_model(proc_device, embedding_model, embedding_tokenizer):
    print("Loading SONAR model...", end='\r')
    if isinstance(proc_device, str):
        normalized_device = proc_device.strip()
        if normalized_device == 'cuda' and not torch.cuda.is_available():
            print("CUDA no disponible; usando CPU para SONAR.")
            normalized_device = 'cpu'
        proc_device = torch.device(normalized_device)
...
def emb_file(in_list, in_numpy, sonar_language):
    ctr = 0
    for i in in_list:
        ctr += 1
        with torch.no_grad():
            embeddings = model.predict(i, source_lang=sonar_language)
        embeddings = ensure_tensor(embeddings)
...
def create_emb_file(infile, sonar_language):
    sentence_list = create_list(infile)
    embs_numpy = emb_file(sentence_list, None, sonar_language)
    f_out = open(infile + embedding_file_suffix, 'wb')
...
def open_emb_file(file_name):
    ...
    embs = np.load(file_name + embedding_file_suffix, allow_pickle=True)
...
def similarity_matrix(embeddings_1, embeddings_2):
    normalized_embeddings_1 = F.normalize(embeddings_1, p=2, dim=1)
    normalized_embeddings_2 = F.normalize(embeddings_2, p=2, dim=1)
...
def score_labse_matrix(source_list, target_list):
    with torch.no_grad():
        source_embeddings = model.predict(source_list, source_lang=source_sonar_language)
        target_embeddings = model.predict(target_list, source_lang=target_sonar_language)
    source_embeddings = ensure_tensor(source_embeddings)
    target_embeddings = ensure_tensor(target_embeddings)
...
def create_anchor_files(file_name, sentence_overlap, lengthfilter):
    ...
    src_embeddings_file = src_overlaps_file + embedding_file_suffix
    trg_embeddings_file = trg_overlaps_file + embedding_file_suffix
    ...
    with open(anchor_overlaps_source_file + str(sentence_overlap) + '.used', 'w') as anchor_src_overlaps_file, open(anchor_overlaps_target_file + str(sentence_overlap) + '.used', 'w') as anchor_trg_overlaps_file, open(anchor_overlaps_source_file + str(sentence_overlap) + '.linenumbers', 'w') as anchor_src_overlaps_file_linenumbers, open(anchor_overlaps_target_file + str(sentence_overlap) + '.linenumbers', 'w') as anchor_trg_overlaps_file_linenumbers:
        embs = np.load(src_overlaps_file + embedding_file_suffix, allow_pickle=True)
        ...
        embs = np.load(trg_overlaps_file + embedding_file_suffix, allow_pickle=True)
        curr_emb_dict = {}
...
def process_file(filename, minimum_labse_anchor):
    ...
    ## create SONAR embeddings ##
    create_emb_file(temporary_folder + '/overlaps.' + file_name + '.src', source_sonar_language)
    create_emb_file(temporary_folder + '/overlaps.' + file_name + '.trg', target_sonar_language)
    ...
    if source_len > 0 and target_len > 0:
                ...
                try:
                    ...
                    if len(galechurch_alignments) > 0:
                        try:
                            ...
                            processInfo.set_status("Calculating SONAR matrix")
                            try:
        ...
        ## Read SONAR embeddings ##
        processInfo.set_status("Reading SONAR embeddings")
...
if __name__ == '__main__':
    ...
    model = load_sonar_model(args.proc_device, embedding_model_name, embedding_tokenizer_name)
    files2align = {}
    filesdone = {}
